...
class Nested_set {
	 ...
	 */
	public function setControlParams(	$table_name, 
	...
	*/
	...
	public function deleteNode($node) {
		...
		return $this->getNodeWhere($leftcol . " < " . $leftanchor);
	}
	...
	public function getTreeAsHTMLDropDown($fields=array()) {
		return $this->getSubTreeAsDropDownList($this->getRootNodes(), $fields);
	}

	public function getSubTreeAsDropDownList($nodes, $fields = array()) {
		if(isset($nodes[0]) && !is_array($nodes[0])) {
			$nodes = array($nodes);
		}

		$retVal = '';
		$retVal .= '<select class="chosen-select form-control" name="category" id="form-field-select-3" data-placeholder="Mời Chọn...">';
		foreach($nodes AS $node) {
			$tree_handle = $this->getTreePreorder($node);
			...
			while($this->getTreeNext($tree_handle))
			{
				// print indentation
				//$retVal .= (str_repeat('-', $this->getTreeLevel($tree_handle)*4));
				$x = str_repeat('-- ', $this->getTreeLevel($tree_handle)*1);

				// print requested fields
				$field = reset($fields);
				//pre($tree_handle['row']);
				while($field){
					$retVal .= '<option value="'.$tree_handle['row']['id'].'">'.$x.$tree_handle['row'][$field] . "";
					$field = next($fields);
				}
				$retVal .= "</option>";
	...
	public function getTreeAsHTMLDropDown2($chosen, $fields=array()) {
		return $this->getSubTreeAsDropDownList2($this->getRootNodes(), $chosen, $fields);
	}

	public function getSubTreeAsDropDownList2($nodes, $chosen, $fields = array()) {
		if(isset($nodes[0]) && !is_array($nodes[0])) {
			$nodes = array($nodes);
		}
		...
		foreach($nodes AS $node) {
			...
			while($this->getTreeNext($tree_handle))
			{
				...
				while($field){
					$ch = "";
					if($tree_handle['row']['id'] == $chosen) {
						$ch = 'selected';
					} 
					$retVal .= '<option value="'.$tree_handle['row']['id'].'"'.$ch.'>'.$x.$tree_handle['row'][$field] . "";
					$field = next($fields);
				}
				$retVal .= "</option>";
	...
	public function getTreeAsHTMLDropDown3($fields=array()) {
		return $this->getSubTreeAsDropDownList3($this->getRootNodes(), $fields);
	}

	public function getSubTreeAsDropDownList3($nodes, $fields = array()) {
		if(isset($nodes[0]) && !is_array($nodes[0])) {
			$nodes = array($nodes);
		}

		$retVal = '';
		$retVal .= ' <select multiple="" class="chosen-select form-control tag-input-style" name="list_question_cate" id="form-field-select-4" data-placeholder="Chọn danh mục câu hỏi...">';
		foreach($nodes AS $node) {
			$tree_handle = $this->getTreePreorder($node);
	...
	public function getTreeAsHTMLQuestionGroup($fields=array()){
		return $this->getSubTreeQuestionGroup($this->getRootNodes(), $fields);
	}

	public function getSubTreeQuestionGroup($nodes, $fields = array()) {
		if(isset($nodes[0]) && !is_array($nodes[0])) {
			$nodes = array($nodes);
		}
		...
		foreach($nodes AS $node) {
			...
			while($this->getTreeNext($tree_handle))
			{
				...
				// print requested fields
				$field = reset($fields);
				if($tree_handle['row']['id']!=1){
					//pre($tree_handle['row']);
					while($field){
						$retVal .= '<li><a class="openModal" data-toggle="modal" data-target="#editmodal" data-id="'.$tree_handle['row']['id'].'" href="#">'.$x.$tree_handle['row'][$field] . "";
						$field = next($fields);
					}
					$retVal .= '<a id="delete_questions_group" data-id="'.$tree_handle['row']['id'].'" href="#">
